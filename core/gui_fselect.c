...
int gui_eyefi_mode_old = -1;
int gui_eyefi_redraw = 0;
 //-------------------------------------------------------------------
void gui_eyefi_init(int title, const char* dir, void (*on_select)(const char *fn)) {
    int i;
    
    main_w = (1+NAME_SIZE+SPACING+SIZE_SIZE+SPACING+TIME_SIZE+1+1)*FONT_WIDTH;
    main_h = FONT_HEIGHT+4+BODY_LINES*FONT_HEIGHT+4+FONT_HEIGHT;
    main_x = (screen_width-main_w)>>1;
    main_y = (screen_height-main_h)>>1;
    ...
    fselect_on_select = on_select;
    marked_operation = MARKED_OP_NONE;
    gui_eyefi_mode_old = gui_get_mode();
    gui_eyefi_redraw = 2;
    gui_set_mode(GUI_MODE_EYEFI);
}

extern void *eyefi_buf(void);
extern char *eyefi_file_name(int);
extern char *print_card_firmware_info(void);
extern char *locate_eyefi_mount(void);
extern char *msgnr(int nr);

void chdk_print(char *str)
{
	static int place = 0;
	draw_string(main_x+FONT_WIDTH, main_y+FONT_HEIGHT+4+place*FONT_HEIGHT, str, MAKE_COLOR(COLOR_RED, COLOR_WHITE));
	place = (place+1) % 10;
	msleep(100);
}

 ...
 */
void hmac_sha1_vector(const u8 *key, size_teye key_len, size_teye num_elem,
 ...
 * @mac: Buffer for the hash (20 bytes)
 */
void hmac_sha1(const u8 *key, size_teye key_len, const u8 *data, size_teye data_len,
	       u8 *mac)
{
 ...
 */
void sha1_prf(const u8 *key, size_teye key_len, const char *label,
	      const u8 *data, size_teye data_len, u8 *buf, size_teye buf_len)
{
	u8 zero = 0, counter = 0;
	size_teye pos, plen;
	u8 hash[SHA1_MAC_LEN];
	size_teye label_len = os_strlen(label);
 ...
 */
void sha1_t_prf(const u8 *key, size_teye key_len, const char *label,
		const u8 *seed, size_teye seed_len, u8 *buf, size_teye buf_len)
{
 ...
 */
int tls_prf(const u8 *secret, size_teye secret_len, const char *label,
	    const u8 *seed, size_teye seed_len, u8 *out, size_teye outlen)
{
	size_teye L_S1, L_S2, i;
	const u8 *S1, *S2;
...
static void pbkdf2_sha1_f(const char *passphrase, const char *ssid,
			  size_teye ssid_len, int iterations, unsigned int count,
			  u8 *digest)
{
	...
	const u8 *addr[2];
	size_teye len[2];
	size_teye passphrase_len = os_strlen(passphrase);
 ...
 */
void sha1_vector(size_teye num_elem, const u8 *addr[], const size_teye *len,
		 u8 *mac)
{
	SHA1_CTX ctx;
	size_teye i;
...
int fips186_2_prf(const u8 *seed, size_teye seed_len, u8 *x, size_teye xlen)
{
	u8 xkey[64];
	u32 t[5], _t[5];
	int i, j, m, k;
